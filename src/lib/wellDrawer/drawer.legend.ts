...
import { CssVarsConfig, LegendRenderConfig } from '@/src/lib/@types/drawer.types';
...
function resolveCssVar(node: Element | null, varName: string, fallback: string): string {
...
export function drawWellLegend(
  ...
  const cfg = defu(options.config ?? {}, INTERACTIVE_RENDER_CONFIG.legend) as LegendRenderConfig;

  const items: LegendItem[] = [];
  if (hasSimple)  items.push({ kind: 'fracture', label: cfg.labels.fractureSingle, swarm: false, water_intake: false });
  if (hasSwarm)   items.push({ kind: 'fracture', label: cfg.labels.fractureSwarm,  swarm: true,  water_intake: false });
  if (hasWaterFx) items.push({ kind: 'fracture', label: cfg.labels.fractureWater,  swarm: false, water_intake: true  });
  if (hasCaveDry) items.push({ kind: 'cave',     label: cfg.labels.caveDry,                     water_intake: false });
  if (hasCaveWet) items.push({ kind: 'cave',     label: cfg.labels.caveWet,                     water_intake: true  });
  ...
  if (options.cssVars) {
    for (const key of Object.keys(options.cssVars) as (keyof CssVarsConfig)[]) {
      ...
      if (cssVar) svgEl.style(cssVar, options.cssVars[key]);
    }
  }
  ...
  const fractureDryColor = resolveCssVar(node, CSS_VAR_MAP.fractureDryStroke, CSS_VAR_FALLBACKS.fractureDryStroke);
  const fractureWetColor = resolveCssVar(node, CSS_VAR_MAP.fractureWetStroke, CSS_VAR_FALLBACKS.fractureWetStroke);
  const caveDryColor     = resolveCssVar(node, CSS_VAR_MAP.caveDryStroke,     CSS_VAR_FALLBACKS.caveDryStroke);
  const caveWetColor     = resolveCssVar(node, CSS_VAR_MAP.caveWetStroke,     CSS_VAR_FALLBACKS.caveWetStroke);
  ...
  items.forEach((item, i) => {
    ...
    if (item.kind === 'fracture') {
      ...
      const texture = item.water_intake ? DEFAULTS_TEXTURES.cave_wet : DEFAULTS_TEXTURES.cave_dry;
      const rw = 24;
      const rh = 12;
