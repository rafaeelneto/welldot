      ...
      try {
        ...
        table.datas.forEach((row, i) => {
          ...
          table.headers.forEach((dataHeader, index) => {
            ...
            // renderer column
            // renderer && (text = renderer(text, index, i, row, rectRow, rectCell)) // value,
            // index-column, index-row, row  nbhmn
        ...
        table.rows.forEach((row, i) => {
          ...
          // Switch to next page if we cannot go any further because the space is over.
          // For safety, consider 3 rows margin instead of just one
          // if (startY + 3 * rowHeight < maxY) startY = rowBottomY +
          // columnSpacing + rowDistance; // 0.5 is spacing rows
          // else this.addPage();
          if (startY + 2 * rowHeight >= maxY) this.addPage();
          startY = rowBottomY + columnSpacing + rowDistance; // 0.5 is spacing rows
