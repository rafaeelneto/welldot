...
// TODO: remove (src_old ref) // import { innerRenderPdf, printPdf, downloadPdf } from '../../views/PDFExport/pdfGenerate';
...
// TODO: remove (src_old ref) // import { getLithologicalFillList } from '../../../src_old/utils/profileD3.utils';
import {
  Cave,
  Constructive,
  Fracture,
...
export function buildSvgProfiles(props: {
  ...
  const drawLog = (svgInfo: SvgInfo) => {
    ...
    const fracturesGroup = pocoGroup
      .append('g')
      .attr('class', 'fractures-group');
    ...
    const updateGeology = async (data: Lithology[], yScale) => {
      // TODO: remove (src_old ref) — getLithologicalFillList from src_old removed; texture fill disabled
      ...
      layers
        ...
        .style('fill', (d: Lithology) => {
          // TODO: remove (src_old ref) — lithological texture fill disabled
    ...
    const updateLithologyLabels = (data: Lithology[], yScale) => {
      // TODO: remove (src_old ref) — wrap utility from src_old removed; labels use plain text
      ...
      labels
        ...
        // TODO: remove (src_old ref) — .call(wrap, GEOLOGY_TIP_WIDTH) removed
        .attr('y', (d: Lithology, i) => {
          if (i > 0) {
            const lastTextHeight =
              // @ts-ignore
    ...
    const updateCaves = (
      ...
      data.forEach(cave => {
        ...
        const closedPath = `${
          topPath
        } L ${xRight.toFixed(1)},${botPts[botPts.length - 1][1].toFixed(1)}${botReversed.replace(
          /^M [\d.-]+ [\d.-]+/,
          '',
    ...
    const updatePoco = (data: Constructive, yScale) => {
      ...
      wellCaseTip
        ...
        // TODO: remove (src_old ref) — .call(wrap, 50) removed
      ...
      wellScreenTip
        ...
        // TODO: remove (src_old ref) — .call(wrap, 80) removed
    ...
    const updateFractures = (
      ...
      data.forEach(fracture => {
        ...
        if (fracture.swarm) {
          ...
          appendPolyline(
            wavyLine(rng, steps, 0, 2 * fractureScale, insetL, insetR),
            1.8,
          );
        ...
        const azLabel =
          fracture.azimuth != null ? ` Az:${fracture.azimuth}°` : '';
        const tipText = `${depthLabel}${dipLabel}${azLabel}`;
        const estimatedW =
          tipText.length * (LABEL_FONT_SIZE * 0.52) + LABEL_PAD_X * 2;
    ...
    const drawUnitLabels = (
      ...
      const unitLabelGroup = litoligicalGroup
        .append('g')
        .attr('class', 'unit-labels');
  ...
  if (hasFractureLegend || hasCaveLegend) {
    ...
    const allLegendItems: LegendItem[] = [
      {
        kind: 'fracture',
        label: 'Fratura simples',
        color: '#000000',
        swarm: false,
      },
      {
        kind: 'fracture',
        label: 'Enxame de fraturas',
        color: '#000000',
        swarm: true,
      },
      {
        kind: 'fracture',
        label: "Entrada d'água",
        color: '#1a6fa8',
        swarm: false,
      },
    ...
    const legendItems = allLegendItems.filter(
      (_, i) => [hasSimple, hasSwarm, hasWaterFx, hasCaveDry, hasCaveWet][i],
    );
    ...
    const legendSvgEl = divContainer
      .append('svg')
      .attr('id', LEGEND_SVG_ID)
      .attr('width', svgTotalWidth)
      .attr('height', LEGEND_HEIGHT);

    const legendG = legendSvgEl
      .append('g')
