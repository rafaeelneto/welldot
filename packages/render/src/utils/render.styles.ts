...
export function buildSvgStyleBlock(theme: WellTheme): string {
  return [
    ...
    `.wp-annotation-body{font-weight:400;font-family:${theme.labels.bodyFont ?? 'sans-serif'};fill:${theme.labels.bodyColor}}`,
    `.wp-depth-tip-text{font-family:${theme.labels.bodyFont ?? 'sans-serif'};fill:${theme.labels.color}}`,
  ].join('');
}
